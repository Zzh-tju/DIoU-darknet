...
void diounms_sort(detection *dets, int total, int classes, float thresh, char *nms_kind, float beta1)
{
    int i, j, k;
    k = total-1;
    for(i = 0; i <= k; ++i){
        ...
        for(i = 0; i < total; ++i){
            ...
            for(j = i+1; j < total; ++j){
                ...
                if (box_iou(a, b) > thresh && strcmp(nms_kind, "greedynms")==0) {
                    ...
                    if (box_diounms(a, b, beta1) > thresh && strcmp(nms_kind, "diounms")==0){
                      dets[j].prob[k] = 0;
                    }
                }
            }
