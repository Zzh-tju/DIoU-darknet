...
layer parse_yolo(list *options, size_params params)
{
    ...
    char *nms_kind = option_find_str(options, "nms_kind", "greedynms");
    if (strcmp(nms_kind, "greedynms")==0) {
      printf("NMS is set to greedy-NMS\n");
    } else {
      if (strcmp(nms_kind, "diounms")==0) {
        printf("NMS is set to DIoU-NMS\n");
      }
    }
    float beta1 = option_find_float(options, "beta1", 1.0);
    if (strcmp(nms_kind, "diounms")==0) {
      printf("beta1 for DIoU-NMS is set to %f\n",beta1);
    } else {
        printf("beta1 for DIoU-NMS is set to infinity\n");
    }
    ...
    if (strcmp(iou_loss, "mse")==0) {
      ...
      if (strcmp(iou_loss, "giou")==0) {
        ...
        l.iou_loss = GIOU;
          ...
          if (strcmp(iou_loss, "diou")==0) {
          printf("loss is set to DIOU\n");
          l.iou_loss = DIOU;
          } else {
              if (strcmp(iou_loss, "ciou")==0) {
              printf("loss is set to CIOU\n");
              l.iou_loss = CIOU;
              }
          }
        }
    ...
    fprintf(stderr, "Yolo layer params: iou loss: %s, iou_normalizer: %f, cls_normalizer: %f\n", (l.iou_loss==MSE?"mse":(l.iou_loss==IOU?"iou":(l.iou_loss==GIOU?"giou":(l.iou_loss==DIOU?"diou":"ciou")))), l.iou_normalizer, l.cls_normalizer);
