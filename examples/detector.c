...
void validate_detector(char *datacfg, char *cfgfile, char *weightfile, char *outfile, char *prefix_arg)
{
    int j;
    list *options = read_data_cfg(datacfg);
    list *cfgs = read_data_cfg(cfgfile);
    char *nms_kinds = option_find_str(cfgs, "nms_kind", "greedynms"); 
    float beta = option_find_float(cfgs, "beta1", 1.0);
    char *valid_images = option_find_str(options, "valid", "data/train.list");
    char *name_list = option_find_str(options, "names", "data/names.list");
    char *prefix;
    ...
    for(i = nthreads; i < m+nthreads; i += nthreads){
        ...
        for(t = 0; t < nthreads && i+t-nthreads < m; ++t){
            ...
            if (nms) diounms_sort(dets, nboxes, classes, nms, nms_kinds, beta);
            if (coco){
